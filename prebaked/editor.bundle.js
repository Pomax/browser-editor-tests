...
var ViewPlugin = class _ViewPlugin {
  constructor(id2, create3, domEventHandlers, domEventObservers, buildExtensions) {
    this.id = id2;
    this.create = create3;
...
function formatPatch(diff2) {
  ...
  ret.push(
    "--- " + diff2.oldFileName + (typeof diff2.oldHeader === "undefined" ? "" : "	" + diff2.oldHeader)
  );
  ret.push(
    "+++ " + diff2.newFileName + (typeof diff2.newHeader === "undefined" ? "" : "	" + diff2.newHeader)
  );
...
var DirTree = class _DirTree {
  tree = {};
  constructor(files = [], { getFileValue = (filename) => filename, ignore = [] } = {}) {
    files.forEach((file) => this.addFile(file, getFileValue, ignore));
    _DirTree.sort(this.tree);
  }
...
function addFileDropFunctionality() {
  ...
  filetree.addEventListener(`drop`, async function dropHandler(ev) {
    ...
    async function traverseFileTree(item, path) {
      ...
      if (item.isFile) {
        item.file(async (file) => {
          const content2 = await getFileContent(file);
    ...
    ev.preventDefault();
    await Promise.all(
      [...ev.dataTransfer.items].map(
        async (item) => traverseFileTree(item.webkitGetAsEntry())
      )
    );
    setTimeout(refreshDirTree, 1e3);
  });
}
...
function setupEditorTab(filename) {
  const tab = create2(`div`);
  tab.title = filename;
  tab.textContent = filename;
  document.querySelectorAll(`.active`).forEach((e) => e.classList.remove(`active`));
  tab.classList.add(`tab`, `active`);
  const close = create2(`button`);
  close.textContent = `x`;
...
function addEventHandling(filename, panel, tab, close, view) {
  ...
  close.addEventListener(`click`, () => {
    if (tab.classList.contains(`active`)) {
      ...
      const tabPos = tabs2.indexOf(filename);
